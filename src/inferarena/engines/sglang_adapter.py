"""Real-cluster execution engine backed by an SGLang deployment.

SGLang exposes an OpenAI-compatible API by default. This engine connects to that
...
Start SGLang separately, for example::
...
class SGLangEngine(ExecutionEngine):
    """Real-cluster execution engine backed by an SGLang server."""
    ...
    def run(self, spec: ExperimentSpec) -> ExperimentResult:
        """Run the workload against the SGLang deployment and return metrics."""
        try:
            from openai import AsyncOpenAI
        except ImportError as exc:
            raise ImportError(
                "SGLang engine requires the openai package. "
                "Install it with: pip install inferarena[sglang]"
            ) from exc
    ...
    async def _call_api(
        ...
        """Call the SGLang API for a single request and record timings."""
    ...
    def _base_url(self) -> str:
        """Return the SGLang base URL from the engine spec or default."""
        return getattr(self.engine_spec, "base_url", "http://localhost:30000/v1")
    ...
    def _model(self) -> str:
        ...
        return getattr(self.engine_spec, "model", "default")
